...
import { BookOpen, CheckCircle, FileText, Archive, Plus, Trash2, Edit2, Star, Users, ShieldAlert } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredCourses.map(course => (
          <BorderGlow
            key={course.id}
            edgeSensitivity={15}
            glowColor={course.status === 'pending' ? '176 99 34' : '304 76 30'}
            backgroundColor={theme === 'dark' ? '#16171F' : '#FFFFFF'}
            borderRadius={16}
            glowRadius={40}
            glowIntensity={1.8}
      ...
      <AddCourseModal 
        ...
        defaultInstructorName={currentUser?.name}
      />
    </div>
  );
}
