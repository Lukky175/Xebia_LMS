...
function BatchesPage({ searchQuery = '' }) {
  ...
  const approveRequest = (request) => {
    setBatches((current) => current.map((batch) => (
      batch.name === request.batchName ? { ...batch, status: 'Approved' } : batch
    )));
  ...
  const rejectRequest = (request) => {
    setBatches((current) => current.map((batch) => (
      batch.name === request.batchName ? { ...batch, status: 'Rejected' } : batch
    )));
    setRequests((current) => current.filter((item) => item.id !== request.id));
  };
        ...
        <ProfileCardFrame theme={theme} className="p-5 md:p-6">
          ...
          <div className="flex flex-col gap-3 border-t border-medium-grey/40 dark:border-border-card pt-4 mt-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-2 self-end sm:self-auto">
              <button type="button" onClick={() => setPage((current) => Math.max(1, current - 1))} disabled={page === 1} className="rounded-lg border border-medium-grey/40 dark:border-border-card bg-white dark:bg-[#16171F] px-3 py-2 text-xs font-bold text-dark-grey disabled:opacity-40 disabled:cursor-not-allowed">
                Prev
              </button>
              ...
              <button type="button" onClick={() => setPage((current) => Math.min(totalPages, current + 1))} disabled={page === totalPages} className="rounded-lg border border-medium-grey/40 dark:border-border-card bg-white dark:bg-[#16171F] px-3 py-2 text-xs font-bold text-dark-grey disabled:opacity-40 disabled:cursor-not-allowed">
                Next
              </button>
            </div>
          </div>
