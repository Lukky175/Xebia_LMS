    ...
    <aside className={`h-screen bg-white dark:bg-[#11050F] text-black dark:text-white flex flex-col justify-between transition-all duration-300 relative border-r border-medium-grey dark:border-white/5 shadow-lg dark:shadow-2xl ${isCollapsed ? 'w-24' : 'w-64'} ${isMobile ? 'border-r-0 shadow-none' : ''}`}>
      ...
      {!isMobile && (
        <button 
          ...
          className="absolute -right-3 top-6 h-6 w-6 rounded-full bg-cta-orange text-white flex items-center justify-center shadow-lg border border-white/10 hover:scale-125 active:scale-95 transition-all duration-200 cursor-pointer z-50"
        >
                    ...
                    className={({ isActive }) => 
                      `flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all duration-200 cursor-pointer group relative ${
                        ...
                        isActive 
                          ...
                          : 'text-text-secondary hover:bg-bg-hover hover:text-text-primary hover:translate-x-1 dark:text-text-secondary dark:hover:bg-bg-hover dark:hover:text-text-primary'
                      }`
                    }
                  >
                    ...
                    {isCollapsed && (
                      <div className="absolute left-20 scale-0 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all duration-200 origin-left bg-black/90 text-white text-[10px] font-bold px-2.5 py-1.5 rounded-md shadow-md z-50 whitespace-nowrap pointer-events-none">
                        {item.label}
                      </div>
                    )}
                  </NavLink>
      ...
      <div className="p-4 border-t border-medium-grey dark:border-white/5 bg-[#F7F8FC]/50 dark:bg-black/10 flex flex-col gap-2 shrink-0">
        ...
        <div className={`flex items-center gap-3 ${isCollapsed ? 'justify-center' : 'justify-start'}`}>
          <motion.div
            ...
            className="h-10 w-10 rounded-xl bg-gradient-to-tr from-tranquil-velvet to-bright-velvet border-2 border-cta-orange/40 flex items-center justify-center text-white font-extrabold shadow-md shrink-0 uppercase select-none"
          >
            {initials}
          </motion.div>
          {!isCollapsed && (
        ...
        <div className={`flex gap-1.5 mt-2 ${isCollapsed ? 'flex-col items-center' : 'flex-row'}`}>
          <button 
            onClick={toggleTheme}
            className={`p-2.5 bg-[#F7F8FC] dark:bg-white/5 hover:bg-medium-grey/40 dark:hover:bg-white/10 text-dark-grey dark:text-white/80 hover:text-black dark:hover:text-white rounded-xl border border-medium-grey/30 dark:border-white/5 transition-all duration-200 hover:scale-105 active:scale-95 flex items-center justify-center cursor-pointer ${isCollapsed ? 'w-10 h-10' : 'flex-1 gap-1.5 text-xs font-semibold'}`}
            title="Toggle Theme"
          >
          ...
          <button 
            ...
            className={`p-2.5 bg-[#F7F8FC] dark:bg-white/5 hover:bg-medium-grey/40 dark:hover:bg-white/10 text-dark-grey dark:text-white/80 hover:text-red-500 dark:hover:text-red-400 rounded-xl border border-medium-grey/30 dark:border-white/5 transition-all duration-200 hover:scale-105 active:scale-95 flex items-center justify-center cursor-pointer ${isCollapsed ? 'w-10 h-10' : 'flex-1 gap-1.5 text-xs font-semibold'}`}
            title="Exit Portal"
          >
